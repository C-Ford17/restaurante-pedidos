...
export default function AdminMenuPage() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/menu/${itemToDelete}`, {
                method: 'DELETE'
            })
            if (res.ok) {
                fetchData()
                                    ...
                                    <div key={item.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden group hover:shadow-lg transition-all">
                                        ...
                                        <div className="p-4">
                                            <div className="flex justify-between items-start gap-2 mb-1">
                                                <h4 className="font-semibold text-slate-900 dark:text-white line-clamp-1 flex-1">
                                                    {item.name}
                                                </h4>
                                            ...
                                            <div className="flex items-center gap-1.5 mb-2">
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${(item.calculatedStock || 0) <= 0
                                                    ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'
                                                    : (item.calculatedStock || 0) < 10
                                                        ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'
                                                        : 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400'
                                                    }`}>
                                                    {item.calculatedStock ?? 0} {t('waiter.available').toLowerCase()}
                                                </span>
                                                {item.useInventory && (
