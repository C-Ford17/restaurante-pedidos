...
export default function AdminInventoryPage() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/inventory/${itemToDelete}`, { method: 'DELETE' })
            if (res.ok) {
                fetchItems()
            } else {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.item')}</th>
                                <th className="hidden md:table-cell px-3 sm:px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.unit')}</th>
                                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.stock')}</th>
                                <th className="hidden md:table-cell px-3 sm:px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.cost')}</th>
                                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.status')}</th>
                                <th className="px-3 sm:px-6 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('admin.inventory.actions')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-8 text-center text-slate-500">
                                        {t('admin.inventory.noItems') || 'No items found.'}
                                    </td>
                                </tr>
                            ) : (
                                filteredItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="hidden md:table-cell px-3 sm:px-6 py-4 text-sm text-slate-500">{item.unit}</td>
                                        <td className="px-3 sm:px-6 py-4">
                                            ...
                                            {Number(item.currentStock) <= Number(item.minStock) && (
                                                <div className="text-xs text-red-500 flex items-center gap-1 mt-1">
                                                    <AlertCircle size={12} /> {t('admin.inventory.lowStock')}
                                                </div>
                                            )}
                                        </td>
                                        <td className="hidden md:table-cell px-3 sm:px-6 py-4 text-sm text-slate-500">
