        ...
        try {
            ...
            if (response.ok) {
                ...
                setData({
                    ...
                    email: orgData.email || '',
                    tipPercentage: orgData.tipPercentage ?? 10
                })
            }
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
                <div className="space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                            {t('admin.settings.organization.tip_percentage') || 'Porcentaje de Propina Sugerido'} (%)
                        </label>
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                onChange={(e) => setData({ ...data, tipPercentage: parseInt(e.target.value) || 0 })}
                                className="w-24 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
                            />
                            <span className="text-slate-500 text-sm">%</span>
                        </div>
                        <p className="text-xs text-slate-500 mt-1">
                            {t('admin.settings.organization.tip_help') || 'Este porcentaje se sugerirá a los clientes al pagar.'}
                        </p>
                    </div>
